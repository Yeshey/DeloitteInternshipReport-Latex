    ...
    \subsection{Resolver \textit{Defects}}\label{sub:defects}
        ...
        \textit{Defect} original: ``\textit{Stamp Visibility | Broker - The UW stamps should be filtered by the company}''
            ...
            \subsubsubsection*{Análise e Resolução:}
                ...
                \begin{comment}
                    \textbf{Fim - Configuração inicial da tela de stamps:}
                
                    \textbf{Início - adição de um novo stamp**}

                    Dado que o RIL Admin está na tela ``stamps''
                    Quando o RIL Admin seleciona ``novo stamp''
                    Então permitir que o RIL Admin insira o nome de exibição do stamp na janela pop-up ``adicionar novo nome de exibição do stamp''
''
                    Dado que o RIL Admin selecionou ``novo stamp''
                    Quando o RIL Admin insere o nome de exibição do stamp na janela pop-up ``adicionar novo nome de exibição do stamp'' e seleciona confirmar e o nome do stamp já existe na plataforma
                    Então mostrar a seguinte exibição e mensagem de aviso conforme 'Tipo de Erro/Aviso' 'Nome não único inserido' (Código - 105) - Consulte a tabela 'Erros/Avisos de Campo/Registo' na página do Confluence Mensagens de Aviso e Erro - Criar e Editar Hierarquia/Condições de Linha Global

                    Dado que o RIL Admin insere um nome na janela pop-up ``adicionar novo nome de exibição do stamp''
                    Quando o RIL Admin seleciona confirmar e o nome do stamp NÃO existe ainda na plataforma
                    Então exibir todos os campos de dados editáveis necessários para criar um novo stamp no separador de detalhes do stamp (incluindo o separador de imagem e separador de dados)

                    Dado que o RIL Admin selecionou ``novo stamp''
                    Quando o RIL Admin insere um ``nome de exibição do stamp'' válido na janela pop-up ``adicionar novo nome de exibição do stamp'' e seleciona continuar
                    Então predefinir a data ``de'' como a data de hoje

                    Dado que o RIL Admin selecionou ``novo stamp''
                    Quando o RIL Admin insere um ``nome de exibição do stamp'' válido na janela pop-up ``novo nome de exibição do stamp'' e seleciona continuar
                    Em seguida, exibir o status ``ativo'' (por padrão)

                    Dado que o RIL Admin selecionou ``novo stamp''
                    Quando o RIL Admin insere um ``nome de exibição do stamp'' válido na janela pop-up ``novo nome de exibição do stamp'' e seleciona continuar
                    Então exibir o status de publicação como ``não publicado'' (por padrão)

                    Dado que o RIL Admin selecionou ``novo stamp''
                    Quando o RIL Admin insere um ``nome de exibição do stamp'' válido na janela pop-up ``novo nome de exibição do stamp'' e seleciona continuar
                    Então mostrar o nome do stamp como título do painel de detalhes do stamp

                    Dado que o RIL Admin inseriu um ``nome de exibição do stamp'' válido na janela pop-up ``adicionar novo nome de exibição do stamp'' e selecionou continuar
                    Quando o RIL Admin seleciona ``consórcio'' ou ``pool'' como ``tipo de stamp'' no separador de dados do painel de detalhes do stamp
                    Então exibir a seção de membros do stamp no separador de dados do painel de detalhes do stamp

                    Dado que o RIL Admin selecionou ``consórcio'' ou ``pool'' como ``tipo de stamp'' no separador de dados do painel de detalhes do stamp
                    Ao exibir as seções de membros do stamp no separador de dados do painel de detalhes do stamp
                    Então ter uma única linha mostrada, por padrão, com os seguintes campos: nome da organização, nome de exibição do stamp e alocação

                    Dado que o RIL Admin selecionou ``consórcio'' ou ``pool'' como ``tipo de stamp'' no separador de dados do painel de detalhes do stamp
                    Ao exibir as seções de membros do stamp
                    Então permitir que o RIL Admin ``adicionar membro do stamp''

                    Dado que o sistema está exibindo as seções de membros do stamp no separador de dados do painel de detalhes do stamp
                    Quando o RIL Admin seleciona ``adicionar membro do stamp'' dentro da seção de membros do stamp
                    Em seguida, exibir uma nova linha com os seguintes campos: nome da organização, nome de exibição do stamp e alocação e permitir novamente que o RIL Admin ``adicionar membro do stamp''

                    Dado que o sistema está exibindo as seções de membros do stamp no separador de dados do painel de detalhes do stamp
                    Quando o RIL Admin seleciona o menu suspenso ``nome da organização'' dentro da seção de membros do stamp
                    Em seguida, exibir uma lista de todas as organizações de transportadoras ATIVAS

                    Dado que o RIL Admin selecionou ``nome da organização'' dentro da seção de membros do stamp
                    Quando o RIL Admin seleciona uma organização de transportadoras ativa
                    Em seguida, filtrar a lista de ``nome de exibição do stamp'', dentro da seção de membros do stamp, para apenas aqueles vinculados à organização selecionada

                    Dado que o RIL Admin NÃO selecionou uma organização de transportadoras dentro da seção de membros do stamp
                    Quando o RIL Admin seleciona a lista suspensa ``nome de exibição do stamp'' dentro da seção de membros do stamp
                    Em seguida, mostrar todos os stamps ATIVOS na plataforma

                    Dado que o RIL Admin selecionou um ``nome de exibição do stamp'' dentro da seção de membros do stamp
                    Quando o nome da organização não foi selecionado
                    Em seguida, o sistema deve inserir o nome da organização vinculado ao stamp (se apenas uma opção de organização for dada) Nota - Um stamp deve ser único para uma organização, mas não para a plataforma

                    Dado que o RIL Admin seleciona ``adicionar novo stamp''
                    Quando um novo stamp é criado e o utilizador NÃO inseriu todos os dados obrigatórios (incluindo a alocação de consórcios e pools totalizando 100%) e o RIL Admin tenta salvar
                    Em seguida, mostrar a seguinte exibição e mensagem de aviso conforme 'Tipo de Erro/Aviso' 'Percentagem de alocação não igual a 100% para o consórcio' (Código - 117 e 112) - Consulte a tabela 'Erros/Avisos de Campo/Registo' na página do Confluence Mensagens de Aviso e Erro - Criar e Editar Hierarquia/Condições de Linha Global

                    Dado que um novo stamp foi criado e o RIL Admin NÃO inseriu todos os dados obrigatórios (incluindo a alocação de consórcios e pools totalizando 100%) e, portanto, não salvou
                    Quando o RIL Admin tenta sair da tela
                    Em seguida, mostrar a seguinte exibição e mensagem de aviso conforme 'Tipo de Erro/Aviso' 'Tentativa de sair da tela (abas não hierárquicas) sem salvar as alterações' (Código - 114) - Consulte a tabela 'Erros/Avisos de Campo/Registo' na página do Confluence Mensagens de Aviso e Erro - Criar e Editar Hierarquia/Condições de Linha Global

                    Dado que o RIL Admin tenta sair da tela e é apresentado com uma mensagem de aviso
                    Quando o RIL Admin seleciona confirmar
                    Em seguida, NÃO salvar o stamp (ou seja, excluir quaisquer dados e o stamp criado) e permitir que o RIL Admin saia da tela de entrada de dados do ``stamp''

                    Dado que o RIL Admin tenta sair da tela e é apresentado com uma mensagem de aviso
                    Quando o RIL Admin seleciona cancelar
                    Em seguida, devolver o RIL Admin à tela de dados do stamp que criaram (ou seja, não remover quaisquer dados)

                    \textbf{Remover AC}

                    Dado que o RIL Admin seleciona ``adicionar novo stamp''
                    Quando um novo stamp é criado e o RIL Admin inseriu todos os dados obrigatórios
                    Então permitir que o RIL Admin salve o stamp

                    Dado que um novo stamp foi criado e o RIL Admin inseriu todos os dados obrigatórios
                    Quando o RIL Admin salva o stamp
                    Então exibir o stamp na lista geral de stamps com os seguintes itens de dados na tela da lista de stamps: Nome de exibição do stamp, tipo de agência, tipo de stamp, de, para, status, classificação e ação

                    \textbf{Início - adição de um novo stamp**}
                                
                    \textbf{Fim - Adicionando um novo stamp:}
                \end{comment}
